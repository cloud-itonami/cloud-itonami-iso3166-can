  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Kita Systems CA"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Kita Systems CA" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest ca-presence-missing-is-held-and-unoverridable
  (testing "missing Japan-resident authorized representative -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:ca-presence-missing} (-> (store/ledger db) last :basis)))
...
(deftest bn-unverified-is-held-and-unoverridable
  (testing "unverified corporate number when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:bn-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
