# Business Model: Independent Public-Sector Market-Entry & Procurement Compliance Service — Canada

## Classification

- Repository: `cloud-itonami-iso3166-can`
- ISO 3166: `CAN` (Canada)
...
- Social impact: [:indigenous-procurement-strategy-access :public-spend-transparency :cross-border-friction-reduction]

## Customer

- an already-incorporated `cloud-itonami-cofog-{code}` /
  `cloud-itonami-isco-{code}` / `cloud-itonami-unspsc-{segment}` /
  `cloud-itonami-{ISIC}` operator wanting to bid on a Canadian
  public contract
...
- registration walkthrough for CanadaBuys, the federal government's procurement portal
- business/tax registration checklist: a federal registration via Corporations Canada, or provincial incorporation, plus a Business Number (BN) from the CRA; GST/HST registration with the Canada Revenue Agency (CRA) where the taxable-supply threshold is met
- local-content / preferential-procurement navigation: the Procurement Strategy for Indigenous Business (PSIB), which sets mandatory minimum-participation targets and set-asides for Indigenous-owned businesses on federal contracts
...
- this service does **not** provide legal or tax advice; characterization
  ...
  routes to Canadian-licensed counsel or a registered agent
...
- **`cloud-itonami-cofog-{code}`**: a jurisdiction-agnostic operator
  ...
  blueprint AND this one to operate in Canada).
