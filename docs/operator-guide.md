...
2. Register the client's intake: business type, target public function,
   prior filing history in Canada if any.
3. Run the advisor in read-only mode against CanadaBuys, the federal government's procurement portal.
...
- named referral relationship with Canadian-licensed counsel or a
...
a working referral relationship with Canadian-licensed counsel or a
registered agent for whatever licensed representation the law of
Canada requires for actual public-procurement filings.
