(ns marketentry.marketentryllm
  ...
  the CAN public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-ca-presence?=" (:has-ca-presence? e)
                        " bn-verified?=" (:bn-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-ca-presence? e))
                              (:has-ca-presence? e))
                          (or (not (:requires-bn? e))
                              (:bn-verified? e)))
                   0.9 0.3)}))
