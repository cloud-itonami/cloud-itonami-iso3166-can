...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (CAN, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up ca-presence-missing) ==")
    ...
    (println "== filing/submit eng-4 (ca-presence-missing -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up bn-unverified) ==")
    ...
    (println "== filing/submit eng-5 (bn-unverified -> HARD hold) ==")
