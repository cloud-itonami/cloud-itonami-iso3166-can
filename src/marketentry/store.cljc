(ns marketentry.store
  "SSoT for the CAN market-entry compliance actor, behind a `Store`
...
(defn demo-data
  ...
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Kita Systems CA" :portal "CanadaBuys"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-ca-presence? true :has-ca-presence? true
             :requires-bn? true :bn-verified? true
             :drafted? false :submitted? false
             :jurisdiction "CAN" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "CanadaBuys"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-ca-presence? true :has-ca-presence? true
             :requires-bn? true :bn-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Minami Systems" :portal "CanadaBuys"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "CAN" :status :intake}
    "eng-4" {:id "eng-4" :operator "Higashi Export" :portal "CanadaBuys"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-ca-presence? true :has-ca-presence? false
             :requires-bn? true :bn-verified? true
             :drafted? false :submitted? false
             :jurisdiction "CAN" :status :intake}
    "eng-5" {:id "eng-5" :operator "Nishi Logistics" :portal "CanadaBuys"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-ca-presence? true :has-ca-presence? true
             :requires-bn? true :bn-verified? false
             :drafted? false :submitted? false
             :jurisdiction "CAN" :status :intake}
    "eng-6" {:id "eng-6" :operator "Chuo Civic Tech" :portal "CanadaBuys"
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               requires-ca-presence? has-ca-presence?
                               requires-bn? bn-verified?
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    (some? requires-ca-presence?)  (assoc :engagement/requires-ca-presence? requires-ca-presence?)
    (some? has-ca-presence?)       (assoc :engagement/has-ca-presence? has-ca-presence?)
    (some? requires-bn?)    (assoc :engagement/requires-bn? requires-bn?)
    (some? bn-verified?)    (assoc :engagement/bn-verified? bn-verified?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/requires-ca-presence? :engagement/has-ca-presence?
   :engagement/requires-bn? :engagement/bn-verified?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :requires-ca-presence? (boolean (:engagement/requires-ca-presence? m))
     :has-ca-presence? (boolean (:engagement/has-ca-presence? m))
     :requires-bn? (boolean (:engagement/requires-bn? m))
     :bn-verified? (boolean (:engagement/bn-verified? m))
...
(defn kotobase-store
  ...
  [{:keys [db-api seq-fn]}]
  (->KotobaseStore (persist/store {:db-api db-api :actor "marketentry-can"})
