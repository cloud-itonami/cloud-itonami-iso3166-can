(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether a Canada-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Canada-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-ca-presence?
                                       true` (near-universal for Canada
                                       ...
                                       Grounded in Canadian presence or
                                       authorized Canadian representative for
                                       many federal procurements (PSPC /
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(BN/統一資格/CanadaBuys登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- ca-presence-missing-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-ca-presence? true`, INDEPENDENTLY verify
  `:has-ca-presence?` is true -- the flagship genuinely new
  check this vertical adds. CONDITIONAL on the engagement's own
  `:requires-ca-presence?` ground truth (most Canada public
  tenders require it; a pure-foreign-market engagement may not)."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-ca-presence? e))
                 (not (true? (:has-ca-presence? e))))
        [{:rule :ca-presence-missing
          :detail (str subject " はカナダ居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn- bn-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-bn? true`, INDEPENDENTLY check
  `:bn-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-bn? e))
                 (not (true? (:bn-verified? e))))
        [{:rule :bn-unverified
          :detail (str subject " はBN確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (ca-presence-missing-violations request st)
                           (engagement-fee-mismatch-violations request st)
                           (bn-unverified-violations request st)
